...
def install_node():
    ...
    if (answer.upper() == 'MAC'):
        ...
        local('wget http://nodejs.org/dist/v0.10.28/node-v0.10.28.tar.gz')
        local('tar -xvzf node-v0.10.28.tar.gz')
        local('cd node-v0.10.28')
        local('mkdir ~/.node')
        ...
        local('cd ..')
        local('rm -rf node-v0.10.28')
        local('rm node-v0.10.28.tar.gz')
...
def clear(app_name, model_name):
    ...
    """
    local("echo 'DROP TABLE %s_%s;' | psql caform700" % (app_name, model_name))
